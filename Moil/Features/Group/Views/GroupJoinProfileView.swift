...
struct GroupJoinProfileView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 14) { ForEach(colors, id: \.self) { color in Button { selectedColor = color } label: { MoilAvatar(color: color, size: 40).overlay { Circle().stroke(MoilColor.textPrimary, lineWidth: selectedColor == color ? 2 : 0).padding(-5) } } } }
            Spacer()

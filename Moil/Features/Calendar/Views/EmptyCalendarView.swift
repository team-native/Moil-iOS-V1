...
struct EmptyCalendarView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Spacer()
            Image(systemName: "person.3.sequence.fill").font(.system(size: 72)).foregroundStyle(MoilColor.primary.opacity(0.55))
            Text("아직 속한 그룹이 없어요").font(MoilTypography.bold(19)).padding(.top, 26)
            ...
            Spacer()
        }.padding(.horizontal, 32).background(MoilColor.background.ignoresSafeArea())
    }
}
